...
const NurseLoginForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        // Create nurse user object
        const nurseUser = {
          id: `nurse_${Date.now()}`,
          name: username,
          role: 'nurse',
          email: `${username}@nurse.local`,
          username: username
        };

        // Store nurse data in localStorage
        localStorage.setItem('userData', JSON.stringify(nurseUser));
        localStorage.setItem('authToken', 'nurse-token');
        localStorage.setItem('authTokens', JSON.stringify({
          access_token: 'nurse-token',
          refresh_token: 'nurse-refresh-token'
        }));
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          >
            ...
            {isLoading ? 'Signing in...' : 'Sign in as Nurse'}
          </button>
        </div>
      </form>
    </div>
